...
indexTpl.events({
    ...
    'click .serviceAction': function () {
        ...
        if (this._serviceCount == 0 || this._serviceCount == null) {
            FlowRouter.go('rabbit.service', {
                customerId: this.customerId, websiteId: this._id
            });
            alertify.customer(fa("plus", "Service"), renderTemplate(Template.rabbit_serviceInsert));
    ...
    'click .paymentWebsiteAction': function () {
        if (this._paymentWebsiteCount == 0 || this._paymentWebsiteCount == null) {
            FlowRouter.go('rabbit.paymentWebsite', {
                customerId: this.customerId, websiteId: this._id
            });
            alertify.customer(fa("plus", "Payment Website"), renderTemplate(Template.rabbit_paymentWebsiteInsert));
...
var configOnRender = function () {
    // date
    var websiteDate = $('[name="registerDate"]');
